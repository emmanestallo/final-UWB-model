...
def approximate_frequency_at_minus_10(power, frequency, center_index):

    index_before_minus10_lc = np.max(np.where(power[:center_index] < -10)[0])
    index_after_minus10_lc = index_before_minus10_lc + 1 

    index_before_minus10_rc = center_index + np.max(np.where(power[center_index:] > -10)[0])
    index_after_minus10_rc = index_before_minus10_rc + 1


    interp_func_before = interp1d(power[index_before_minus10_lc:index_after_minus10_lc+1], frequency[index_before_minus10_lc:index_after_minus10_lc+1], kind='linear')
    interp_func_after = interp1d(power[index_before_minus10_rc:index_after_minus10_rc+1], frequency[index_before_minus10_rc:index_after_minus10_rc+1], kind='linear')

    freq_at_minus_10_before = interp_func_before(-10)
    freq_at_minus_10_after = interp_func_after(-10) 

    bandWid = np.round(freq_at_minus_10_after, 5) - np.round(freq_at_minus_10_before, 5)
...
def process_data(freqz,valz):
    ...
    bandwidth = approximate_frequency_at_minus_10(power, frequency, center_index)

    return center_frequency, bandwidth

###########################END OF FUNCTIONS#################DO NOT MODIFY ANYTHING ABOVE THIS LINE###########################
...
# the sigma_vals should have the same values as h*(0.n)*1e-12 for some range h and n (in matlab)

directory = 'delay_period_mismatch'
...
ax1.plot(x_values, y_values_bandWidth, marker='o', linestyle='-')
...
ax2.plot(x_values, y_values_fCenter, marker='o', linestyle='-')
